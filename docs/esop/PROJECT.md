...
UX source of truth: [design.md](./design.md)
Log: [LOG.md](./LOG.md)
...
| D10 | A sequential three-screen onboarding (Company, Hiring plan, Grant economics) is not the mode switch D8 forbids, provided **no field's tier or requiredness ever depends on which screen it sits on** — `lib/visibility.ts` remains the sole source of tier, unchanged — and provided **every field from every screen becomes simultaneously visible and editable in one place, `Your model`, the instant results are reached**, so there is no second, hidden "advanced" reading of the same data the way a Simple/Advanced toggle would have created one. This narrows D8's wording only: "never behind a mode switch" now reads as "never a second reading of the same fields," not "never more than one screen." D8's substance (no field ever hidden by a mode a founder didn't choose — tiering is the only thing that hides a field), D7's blank-to-result gate, and D9's minimal required set are unchanged and enforced by the same code as before. Full architecture in [design.md](./design.md). | 2026-08-18 |
| D11 | Seven named `minor` fields (or field groups) — Sector, base annual attrition, the leadership attrition override, vested-options-never-exercised, the post-termination exercise window, the vesting policy (cliff, total vesting period, frequency — one group, not three), and the block of Grant policy fields buffer/comp inflation/value basis/refresh — are a **deliberate, named exception to D9's "always show the live default, marked as an `EstimateMarker`" rule**, not a reversal of it for anything else. Sector is removed from the UI outright (the founder never chooses it; `attrition.sector` keeps whatever `buildSeedInputs` seeds and still labels the PDF report's assumption row). The rest sit behind an `OptionalFieldToggle` — a plain "+ Add X" button, not `CollapsibleSection`'s `<details>`, since there is nothing to re-hide once added — collapsed by default; **the engine's D6 default for each keeps running, unmarked and un-badged, until the founder clicks to add it**, which is exactly the unmarked default D6 and D9 exist to prevent, accepted here as a conscious, scoped trade for a shorter default screen rather than an oversight. No field named here is ever `drivesPool` (D9) — confirmed against `lib/visibility.ts` before this shipped — so D7's blank-to-result gate is untouched: none of these disclosures can withhold a result the founder is owed. Grant per hire and strike policy/theta, also in `GrantPolicyCard`, are unaffected and stay gated only by their existing `hideGrantPerHire`/`hideStrikeAndTheta` props, because strike policy *can* be `drivesPool` in a configuration this card does not currently render it in — left out of the toggle on purpose, not missed. `rounds.enabled` (`FundingRoundCard`'s "Add next funding round") is **not** part of this exception and needed none: it was never `EstimateMarker`'d to begin with (an empty `rounds` array is not a `DEFAULTS` value standing in for the founder's — see the card's own comment), so collapsing its disclosure behind `OptionalFieldToggle.onAdd` is a plain UI simplification, not a D9 departure. | 2026-08-18 |
| D12 | The "Why this number" results card's right-hand column — every `minor`-tier assumption the founder didn't set (buffer, attrition, the leaver assumptions, vesting, strike, theta, refresh…) — carries **no field name and no value, ever, on screen, for free**, even though D3 says results themselves are never gated. The card as first built (design.md §5.2's original text) named each one and linked straight into `Your model`, which is a narrower reading of D3 than the report gate itself gets: the report requires a name and a work email (D3/the lead form); this card required nothing. `WhyThisNumber.tsx`'s right-hand list is now a fixed set of opaque, `aria-hidden` bars with no interpolated text of any kind, and the only way past it is `Download full report` — the same lead-gated action every other report surface sits behind. The left-hand "You told us" list is unaffected in shape, but gained one correction: `hiring.seniorityMix.leadership` is `minor` with a non-zero seeded default, so the leadership-hires line now also checks `touched` before crediting a founder with a figure they may never have entered — every other line on that side reads a `drivesPool` field (D9) and needs no such check. `lib/visibility.ts`'s tiers and `src/lib/esop` are both untouched; this is a UI-only narrowing, recorded here because a future session reintroducing a free preview of `minor` values — a "View all assumptions" link, a count, a name — would be undoing this decision rather than adding a feature. | 2026-08-18 |
| D13 | **Extends D12 to the cap table.** `CapTablePanel.tsx`'s "After the recommended pool is reserved" table, and `OwnershipImpact.tsx`'s "After pool"/"Change" columns for the Founders and Investors rows, are locked the same way — unlockable only by `Download full report`. The reason is sharper here than a general D3 narrowing: the founders/investors split on *every* cap table, `before` included, is priced off `company.founderOwnershipPctOfFullyDiluted`, a `reportOnly` field (D9) whose seed is an **invented example fact** (D9's own text names "55% founder ownership" as exactly this kind of invention) until a founder fills in section 07. Presenting it as a modelled result — which is what a founder-facing percentage next to a headline pool number implies — is the unmarked-default problem D6/D9 exist to prevent, one output removed. Unlike D12's column, the locked table's own column headers (`Holder`/`Shares`/`% of fully diluted`) render for real: they name no company or figure, so hiding them bought nothing and made the teaser read as broken rather than locked — only the body, one `aria-hidden` bar standing in for every row, is opaque. `before` ("Today") stays open: that table asserts the *current* cap table as a fact the founder is expected to know, not a modelled outcome, and its Founders/Investors split carries the same caveat via the panel's existing "editable estimate" disclaimer. The "Pool" row in `OwnershipImpact` stays open on both sides for a different reason: its "after" value is exactly the headline recommended-pool percentage, already free everywhere per D3 — locking a restatement of a number sitting large and unlocked one scroll up would hide nothing. `afterModelledRound` (the third `CapTablePanel` table, and `FundingRoundImpact.tsx`) is untouched by this decision; it was not part of what was asked and is worth its own look before being folded in, since a modelled round compounds the same invented split further. | 2026-08-18 |
| D14 | **Extends D12/D13 to `ScenarioStrip.tsx`.** The Slow/Fast scenario cards' detail block — "Current pool", "Founder cost" and the perturbation note ("Hiring at 70%, growth halved, attrition +5 points") — is locked the same way; Base's is not. Two reasons stack here, not one: `poolCostFor` reads the same invented `founderOwnershipPctOfFullyDiluted` split D13 already locks on the cap table, and the note names the specific `minor`-tier perturbation (`lib/scenarios.ts`'s hiring/growth/attrition/grant-size multipliers) — exactly the kind of assumption value D12 locks in "Why this number," just phrased as a scenario description instead of a field label. Base stays fully open: "Your inputs, exactly as entered" names nothing invented, and its figures are the same ones already free in the headline and the (open) `before` cap table. The headline percentage and the `Load` action are unaffected on every card — the recommendation itself is never gated, only what explains or reproduces it. | 2026-08-18 |
| D15 | **Every on-screen chart is locked behind the report download**, per an explicit founder instruction going further than D12–D14's own reasoning: unlike a cap table row or a scenario note, a chart's plotted series is not obviously tied to an invented `reportOnly` fact — this is a deliberate, broader narrowing of D3, not another instance of the D9-default leak D12–D14 all trace back to. `ChartFrame.tsx` (the one shared wrapper all four charts render through — `PoolRunwayChart`, `PoolPctChart`, `HiresSupportedChart`, `GrantCostChart`) gained a `locked` prop: true hides the legend, the caption and the sr-only accessible data table, and renders the real chart `children` **blurred** (`blur-md`) inside a `pointer-events-none`, `aria-hidden` wrapper — a flat opaque placeholder was tried first and rejected by the same founder instruction that asked for it, reading as broken rather than locked; a genuinely blurred chart shows there is something there without giving up an exact figure. `pointer-events-none` is load-bearing, not decoration: Recharts' own hover tooltip renders a precise value in plain text, and a blur filter does not stop a mouse event from reaching the element underneath it, so a hoverable blurred chart would leak through the one interaction the CSS effect can't touch. A visible "Download the report to see this chart" message sits on top, unblurred; the heading renders unconditionally either way. `ResultsPanel.tsx` passes `locked` on all four; **`ReportCharts.tsx` — the off-screen tree `captureChart` rasterises for the PDF the download itself produces — does not**, and must never, or the report a founder pays a name and email for would show the same blur the free screen does. One component (`ChartFrame`), one guard, both call sites reduced to passing a boolean, per the M22 pattern this codebase already holds engine code to. | 2026-08-18 |
...
**2026-08-18 retune, tokens only, still under [024]'s seam.** The vendored `incentiv-design-system/`
bundle above had drifted from what `incentiv.finance/tools/*` actually ships — its own light
`--accent` is still `#1F4FFF`, 8px buttons/inputs, a 12px card, `--font-display: 'Inter'` — so
`tokens/colors.css`, `tokens/shape.css` and `tokens/typography.css`'s `--font-display` were
re-sourced from the live site instead (computed styles, both themes), and `tokens/README.md` now
says so in place of "copy verbatim." Full values, the two accessibility corrections this required,
and the updated chart-palette contrast numbers are in `globals.css` and `lib/chartTheme.ts`.

- **Card radius went to 16px, not the live site's other answer.** Incentiv's own tool pages
  disagree: the ESOP Tax Calculator's panel is a flat 4px with no shadow, the Funding Simulator's
  card is 16px with a soft drop shadow. 16px-plus-shadow was the deliberate pick (asked of, and
  made by, the person driving this pass) — `--r-card` and the newly-real `--shadow-panel` both
  follow it. `--r-button`/`--r-input` follow the one thing both tools agree on, 4px.
- **`--font-mono` still has no consumer, unchanged from [024].** IBM Plex Mono is Incentiv's own
  stat-number typeface on every live tool page and is still not applied anywhere in this route —
  wiring it up is a JSX change to specific number displays, not a token value, so it stayed out of
  scope for a tokens-only pass.
- **`--font-display` now genuinely holds Incentiv's value ('DM Serif Display', Georgia, serif)
  instead of being forced back to Inter, but nothing here reads it yet either** — the same shape of
  gap as `--font-mono`, and for the same reason left open.
