...
Raised in [012], the compliance session:

- `EngineWarningId.cliffBelowStatutoryMinimum` is now dead by design, because the engine blocks the input rather than warning about it. The whole `EngineWarningId` union still has no producer, `solverDidNotConverge` included.
- The Ind AS 102 schedule excludes options granted before year 0 and returns the count it excluded. Valuing them needs a grant-date price per share the engine does not hold. A company with a large existing grant book will see an expense that is understated by exactly that omission, and the fix is an input, not a formula.
- Spec output items 3, 6, 10 and 11 still have no producer, and `EsopOutputs` has none at all. `isTaxDeferralAvailable` now exists for item 11's `taxDeferralAvailable` to be derived from, correctly, when something builds it.
- The compliance checks are not wired to the roll forward. `runComplianceChecks` takes an `AuthorisedCapitalHeadroom` and nothing yet passes the roll forward's own. That join belongs to whatever assembles `EsopOutputs`.
