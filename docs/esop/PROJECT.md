...
| D15 | **Every on-screen chart is locked to its heading alone**, per an explicit founder instruction going further than D12–D14's own reasoning: unlike a cap table row or a scenario note, a chart's plotted series is not obviously tied to an invented `reportOnly` fact — this is a deliberate, broader narrowing of D3, not another instance of the D9-default leak D12–D14 all trace back to. `ChartFrame.tsx` (the one shared wrapper all four charts render through — `PoolRunwayChart`, `PoolPctChart`, `HiresSupportedChart`, `GrantCostChart`) gained a `locked` prop: true hides the legend, the plotted `children`, the caption and the sr-only accessible data table alike, replacing them with an opaque blurred block and a visible "Download the report to see this chart" message; the heading renders unconditionally either way. `ResultsPanel.tsx` passes `locked` on all four; **`ReportCharts.tsx` — the off-screen tree `captureChart` rasterises for the PDF the download itself produces — does not**, and must never, or the report a founder pays a name and email for would show the same blur the free screen does. One component (`ChartFrame`), one guard, both call sites reduced to passing a boolean, per the M22 pattern this codebase already holds engine code to. | 2026-08-18 |
